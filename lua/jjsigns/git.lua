...
function Obj:run_blame(contents, lnum, revision, opts)
  return require('jjsigns.git.blame').run_blame(self, contents, lnum, revision, opts)
end
...
function Obj:stage_hunks(hunks, invert)
  ...
  local relpath = assert(self.relpath)
  local patch = require('jjsigns.hunks').create_patch(relpath, hunks, self.mode_bits, invert)
...
function Obj.new(file, revision, encoding, gitdir, toplevel)
  ...
  if jjroot and cwd and jj.prefers_native(jjroot, cwd) then
    ...
    local jjobj = require('jjsigns.jj.obj').new(fileabs, revision, encoding, jjroot)
    return jjobj --[[@as Jjsigns.GitObj?]]
  end
