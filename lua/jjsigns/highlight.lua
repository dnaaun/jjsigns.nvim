...
--- Export for docgen
--- @type table<string,Jjsigns.Hldef>[]
M.hls = {}
...
---@return Jjsigns.Hldef? spec
local function gen_hl(staged, kind, ty)
  local cty = capitalise(ty)
  local hl = ('JjSigns%s%s%s'):format(staged and 'Staged' or '', cty, kind)
  ...
  elseif kind == 'Cul' then
    fallbacks = { ('JjSigns%s'):format(cty) }
  else
    fallbacks = {
...
vim.list_extend(M.hls, {
  ...
  { JjSignsCurrentLineBlame = { 'NonText', desc = 'Used for current line blame.' } },
  ...
  -- Currently unused
  -- {JjSignsAddLnVirtLn = {'JjSignsAddLn'}},
  -- {JjSignsChangeVirtLn = {'JjSignsChangeLn'}},
  -- {JjSignsAddLnVirtLnInLine = {'JjSignsAddLnInline', }},
  -- {JjSignsChangeVirtLnInLine = {'JjSignsChangeLnInline', }},
...
local function dprintf(fmt, ...)
  dprintf = require('jjsigns.debug.log').dprintf
  dprintf(fmt, ...)
end

--- @param hl string
--- @param hldef Jjsigns.Hldef
...
--- Setup a JjSign* highlight by deriving it from other potentially present
--- highlights.
...
do --- temperature highlight
  ...
  function M.get_temp_hl(min, max, t, alpha, fg)
    local Color = require('jjsigns.color')
    ...
    local hl_name = ('JjSignsColorTemp.%s.%d'):format(fgs, color)
...
do -- Module-level activation
  ...
  api.nvim_create_autocmd('ColorScheme', {
    group = api.nvim_create_augroup('jjsigns.highlight', {}),
