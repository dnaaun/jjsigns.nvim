...
local asystem = async.wrap(3, require('jjsigns.system').system)
...
--- working-copy parent (`@-`). For those repositories jjsigns' existing git
...
--- functions here additionally drive a native backend (see `jjsigns.jj.repo`,
--- `jjsigns.jj.obj`, `jjsigns.jj.blame`) built entirely on `jj` commands.
---
--- The "staging" concept does not exist in jj, so staging actions are disabled
--- for jj repositories (see `Jjsigns.GitObj:supports_staging`).
---
--- All commands run with `--ignore-working-copy` so jjsigns never snapshots or
...
---
--- `--ignore-working-copy` is always passed so that jjsigns never snapshots or
--- locks the user's working copy; we only ever read metadata.
...
--- @param revset string
--- @param relpath string
--- @return Jjsigns.JJ.AnnotateLine[]? lines, string? err
