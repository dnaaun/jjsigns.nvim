...
local ns_removed = api.nvim_create_namespace('jjsigns_removed')

local VIRT_LINE_LEN = 300
local REMOVED_VIRT_LINE_HL = 'JjSignsDeleteVirtLn'
local REMOVED_INLINE_HL = 'JjSignsDeleteVirtLnInLine'
...
local states = {} --- @type table<integer, {hunks: Jjsigns.Hunk.Hunk[], lines: table<Jjsigns.Hunk.Hunk, Jjsigns.CapturedLine[]>, source_bufs: table<string, Jjsigns.HunkPreview.SourceBuf>, pending?: table<Jjsigns.Hunk.Hunk, true>, scheduled?: boolean}>

--- @param str string
--- @param highlights {group?:string, groups?:string[], start:integer}[]
--- @return Jjsigns.VirtTextChunk[]
local function chunks_from_statusline(str, highlights)
  local chunks = {} --- @type Jjsigns.VirtTextChunk[]
...
--- @param win integer
--- @param lnum integer
--- @param opts? {extra_hl?: Jjsigns.HlName|Jjsigns.HlStack}
--- @return Jjsigns.VirtTextChunk[]
local function build_prefix(win, lnum, opts)
  ...
  if extra_hl then
    for _, chunk in ipairs(chunks) do
      local groups = {} --- @type Jjsigns.HlStack
...
local function get_win_ns(winid, bufnr)
  ...
  if not entry then
    entry = {
      ns = api.nvim_create_namespace(('jjsigns_removed_win_%d'):format(winid)),
    }
    win_ns[winid] = entry
...
local function render_virt_lines(lines, start_lnum, opts)
  ...
  local prefix --- @type fun(line_index: integer, line: Jjsigns.CapturedLine): Jjsigns.VirtTextChunk[]?
  if opts.lno_hl then
    local win = assert(opts.win)
    prefix = function(line_index)
      return build_prefix(win, start_lnum + line_index - 1, {
        extra_hl = 'JjSignsVirtLnum',
      })
    end
  end
...
--- @param captured Jjsigns.CapturedLine[][]
...
--- @param source_bufs table<string, Jjsigns.HunkPreview.SourceBuf>?
...
--- @param bufnr integer
--- @param state {hunks: Jjsigns.Hunk.Hunk[], lines: table<Jjsigns.Hunk.Hunk, Jjsigns.CapturedLine[]>, source_bufs: table<string, Jjsigns.HunkPreview.SourceBuf>, pending?: table<Jjsigns.Hunk.Hunk, true>, scheduled?: boolean}
local function flush_pending_entries(bufnr, state)
  ...
  local missing = {} --- @type Jjsigns.Hunk.Hunk[]
...
function M.on_win(winid, bufnr, topline, botline)
  ...
  local visible = {} --- @type {row: integer, above: boolean, start_lnum: integer, lines: Jjsigns.CapturedLine[]}[]
  local missing = {} --- @type Jjsigns.Hunk.Hunk[]
  for _, hunk in ipairs(state.hunks) do
    local row, above = show_deleted_placement(hunk)
...
do -- Module-level activation
  ...
  api.nvim_create_autocmd('WinClosed', {
    group = api.nvim_create_augroup('jjsigns.deleted_preview', {}),
