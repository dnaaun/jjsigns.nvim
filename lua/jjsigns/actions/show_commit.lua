...
local function goto_action(base, bufnr, commit_buf, ref_list, ref_list_ptr)
  ...
  elseif curline:match('^[%+%-]') then
    ...
    Async.run(function()
      require('jjsigns.actions.diffthis').show(bufnr, commit, added and new_file or old_file)
...
function M.show_commit(base, open, bufnr, ref_list, ref_list_ptr)
  ...
  if not res[6] then
    require('jjsigns.message').warn('show_commit is not supported in this repository')
    return
  end
