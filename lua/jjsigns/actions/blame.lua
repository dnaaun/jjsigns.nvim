...
local get_temp_hl = require('jjsigns.highlight').get_temp_hl

local api = vim.api

local hash_colors = {} --- @type table<integer,string>

local ns = api.nvim_create_namespace('jjsigns_blame_win')
local ns_hl = api.nvim_create_namespace('jjsigns_blame_win_hl')
...
local function get_hash_color(sha)
  ...
  local hl_name = string.format('JjSignsBlameColor.%s%s%s', r, g, b)
...
--- @param blame Jjsigns.CacheEntry.Blame
--- @param win integer
...
local function render(blame, win, main_win, buf_sha)
  ...
  for i, b in ipairs(entries) do
    ...
    if sha == last_sha then
      ...
      line_chunks = { { graph, hash_hl } } --- @type Jjsigns.BlameFmtChunk[]
...
--- @async
--- @param opts Jjsigns.BlameOpts?
--- @param blame table<integer,Jjsigns.BlameInfo?>
--- @param win integer
...
local function reblame(opts, blame, win, revision, parent)
  ...
  local did_attach = require('jjsigns.actions.diffthis').show(nil, sha)
...
local function show_commit(win, bwin, open, bcache)
  ...
  api.nvim_set_current_win(win)
  require('jjsigns.actions.show_commit')(sha, open)
end
...
local function diff(bufnr, blm_win, blame)
  ...
  require('jjsigns.actions.diffthis').show(bufnr, info.commit.sha, info.filename)
  if info.previous_sha then
    require('jjsigns.actions').diffthis(info.previous_sha)
  end
end
...
--- @param blm_win integer
--- @param entries table<integer,Jjsigns.BlameInfo?>
--- @param min_time integer
--- @param max_time integer
...
function M.blame(opts)
  ...
  api.nvim_buf_set_name(blm_bufnr, (bcache:get_rev_bufname():gsub('^jjsigns:', 'jjsigns-blame:')))
  ...
  local group = api.nvim_create_augroup('JjsignsBlame', {})
