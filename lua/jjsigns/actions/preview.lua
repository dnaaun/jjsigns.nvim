...
--- @class jjsigns.preview
local M = {}

local ns_inline = api.nvim_create_namespace('jjsigns_preview_inline')
...
local function show_added(bufnr, nsw, hunk)
  ...
  local _, added_regions =
    require('jjsigns.diff_int').run_word_diff(hunk.removed.lines, hunk.added.lines)
  ...
  for _, region in ipairs(added_regions) do
    ...
    api.nvim_buf_set_extmark(bufnr, nsw, start_row + offset, scol, {
      ...
      hl_group = rtype == 'add' and 'JjSignsAddInline'
        or rtype == 'change' and 'JjSignsChangeInline'
...
function M.preview_hunk_inline()
  ...
  api.nvim_create_autocmd({ 'CursorMoved', 'InsertEnter', 'BufLeave' }, {
    buffer = bufnr,
    desc = 'Clear jjsigns inline preview',
    callback = function()
      clear_preview_inline(bufnr)
    end,
    once = true,
