...
--- @class Jjsigns.InspectCompatFilter
...
--- @class Jjsigns.InspectCompatCapture
...
--- @return Jjsigns.HlName|Jjsigns.HlStack|nil
--- @return string?
...
--- @return Jjsigns.InspectPosExtmarkItem
local function extmark_to_map(extmark, nsmap)
  ...
  } --- @type Jjsigns.InspectPosExtmarkItem

  return resolve_hl(item) --[[@as Jjsigns.InspectPosExtmarkItem]]
end
...
--- @param row integer
--- @param cols integer[]
--- @return Jjsigns.InspectPosTSItem[]
local function collect_treesitter(bufnr, row, cols)
  local items = {} --- @type Jjsigns.InspectPosTSItem[]
  local previous = {} --- @type table<string, Jjsigns.InspectPosTSItem>

  for i = 1, #cols - 1 do
    local col = assert(cols[i])
    local next_col = assert(cols[i + 1])
    local active = {} --- @type table<string, Jjsigns.InspectPosTSItem>

    for _, capture in pairs(vim.treesitter.get_captures_at_pos(bufnr, row, col)) do
      --- @cast capture Jjsigns.InspectCompatCapture
...
--- @param row integer
--- @param col integer
--- @param opts? Jjsigns.InspectCompatFilter
--- @return Jjsigns.InspectPosResult
function M.inspect_pos(bufnr, row, col, opts)
  opts = vim.tbl_deep_extend('force', defaults, opts or {}) --[[@as Jjsigns.InspectCompatFilter]]

  local has_range = opts.end_row ~= nil and opts.end_col ~= nil
  if has_range and opts.end_row ~= row then
    error('jjsigns inspect backend only supports single-row ranges')
  end
  ...
  --- @type Jjsigns.InspectPosResult
  local result = {
    treesitter = {}, --- @type Jjsigns.InspectPosTSItem[]
    syntax = {}, --- @type Jjsigns.InspectPosItem[]
    extmarks = {}, --- @type Jjsigns.InspectPosExtmarkItem[]
    semantic_tokens = {}, --- @type Jjsigns.InspectPosExtmarkItem[]
    buffer = bufnr,
    row = row,
    col = col,
  }
