local Status = require('jjsigns.status')
local async = require('jjsigns.async')
local git = require('jjsigns.git')
local Cache = require('jjsigns.cache')
...
--- @class jjsigns.attach
local M = {}

local group = api.nvim_create_augroup('jjsigns.attach', {})
...
local function parse_git_path(name)
  if not vim.startswith(name, 'fugitive://') and not vim.startswith(name, 'jjsigns://') then
    return
  end
...
--- @class (exact) Jjsigns.GitContext
...
M.attach = throttle_async({ hash = attach_hash }, function(opts)
  ...
  if config.current_line_blame then
    require('jjsigns.current_line_blame').update(cbuf)
  end
end)
...
do -- Module-level activation
  api.nvim_create_autocmd('BufFilePre', {
    group = group,
    desc = 'Jjsigns: detach when changing buffer names',
  ...
  api.nvim_create_autocmd('VimLeavePre', {
    desc = 'Jjsigns: detach from all buffers',
    group = group,
    callback = function()
      M.detach_all()
    end,
