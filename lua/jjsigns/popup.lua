...
--- @alias Jjsigns.VirtTextChunk [string, Jjsigns.HlName|Jjsigns.HlStack]

--- Each element represents a multi-line segment
--- @alias Jjsigns.LineSpec [string, string|Jjsigns.HlMark[], string?][]

--- @param ret Jjsigns.HlMark[]
--- @param hlmarks Jjsigns.HlMark[]
...
--- Partition the text and Jjsigns.HlMarks from a Jjsigns.LineSpec[]
--- @param fmt Jjsigns.LineSpec[]
--- @return string[]
--- @return Jjsigns.HlMark[]
local function partition_linesspec(fmt)
  local lines = {} --- @type string[]
  local ret = {} --- @type Jjsigns.HlMark[]
...
local function close_all_but(id)
  for _, winid in ipairs(api.nvim_list_wins()) do
    if vim.w[winid].jjsigns_preview ~= nil and vim.w[winid].jjsigns_preview ~= id then
...
function M.close(id)
  for _, winid in ipairs(api.nvim_list_wins()) do
    if vim.w[winid].jjsigns_preview == id then
      pcall(api.nvim_win_close, winid, true)
    end
  end
end

local ns = api.nvim_create_namespace('jjsigns_popup')

--- @param bufnr integer
--- @param lines_spec Jjsigns.LineSpec[]
...
local function create_win(bufnr, opts, id)
  ...
  vim.w[winid].jjsigns_preview = id
