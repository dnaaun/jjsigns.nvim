...
--- @class jjsigns.main: jjsigns.actions,jjsigns.attach,jjsigns.debug
local M = {}

local cwd_watcher ---@type uv.uv_fs_event_t?

local function log()
  return require('jjsigns.debug.log')
end

local function config()
  return require('jjsigns.config').config
end

local function async()
  return require('jjsigns.async')
end
...
local function get_gitdir_and_head()
  ...
  if package.loaded['jjsigns.cache'] then
    for _, bcache in pairs(require('jjsigns.cache').cache) do
  ...
  local info = require('jjsigns.git').Repo.get_info(cwd)
  ...
  local update_head = debounce_trailing(100, function()
    async()
      .run(function()
        ...
        async().schedule()
        if new_head ~= vim.g.jjsigns_head then
          vim.g.jjsigns_head = new_head
...
local function setup_cli()
  api.nvim_create_user_command('Jjsigns', function(params)
    async().run(function()
      require('jjsigns.cli').run(params)
    end)
  end, {
    force = true,
    nargs = '*',
    ...
    complete = function(arglead, line)
      return require('jjsigns.cli').complete(arglead, line)
    end,
  })
end
...
local function setup_attach()
  ...
  api.nvim_create_autocmd({ 'BufFilePost', 'BufRead', 'BufNewFile', 'BufWritePost' }, {
    ...
    callback = function(args)
      ...
      require('jjsigns.actions').attach({ bufnr = bufnr, trigger = args.event })
    end,
  })
  ...
  api.nvim_create_autocmd({ 'QuickFixCmdPre', 'QuickFixCmdPost' }, {
    group = 'jjsigns',
    pattern = '*vimgrep*',
    desc = 'Jjsigns: disable attach during vimgrep',
    callback = function(args)
  ...
  if config().auto_attach then
    for _, buf in ipairs(api.nvim_list_bufs()) do
      if api.nvim_buf_is_loaded(buf) and api.nvim_buf_get_name(buf) ~= '' then
        ...
        -- attaches is aborted.
        require('jjsigns.actions').attach({ bufnr = buf, trigger = 'setup' })
      end
    end
  end
end
...
--- Setup and start Jjsigns.
---
--- @param cfg table|nil Configuration for Jjsigns.
---     See |jjsigns-usage| for more details.
function M.setup(cfg)
  if vim.fn.executable('git') == 0 then
    print('jjsigns: git not in path. Aborting setup')
    return
  end

  if cfg then
    require('jjsigns.config').build(cfg)
  end

  -- Only do this once
  if init then
    api.nvim_create_augroup('jjsigns', {})
    setup_cli()
    -- Highlights are global editor state, so define them during setup even
    -- when attach-time rendering features stay lazy.
    require('jjsigns.highlight')
    setup_attach()
    setup_cwd_head()
...
function M.statuscolumn(bufnr, lnum)
  return require('jjsigns.sign_renderer').statuscolumn(bufnr, lnum)
end
...
M = setmetatable(M, {
  __index = function(_, f)
    ...
    if config().debug_mode then
      local debug = require('jjsigns.debug')
      if debug[f] then
        return debug[f]
      end
    end
