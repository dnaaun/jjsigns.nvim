...
- Core Lua modules live in `lua/jjsigns/` (config, handlers, utilities).
- Repo maintenance scripts live in `scripts/`.
- User-facing commands are wired in `plugin/jjsigns.vim`
...
- `make doc` / `make doc-check`: regenerate help from `lua/jjsigns/config.lua` and fail if docs drift.
